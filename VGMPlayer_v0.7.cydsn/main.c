...
#define FILL_CURRENT_BUF(i) bufPtr = buffer[i]; \
    while (!rx_flag); \
...
uint8_t buffer[BUF_CMDS][USBUART_BUFFER_SIZE];
uint8_t isrFlag = 0;
...
int main(void) {
    ...
    //uint32_t test = 0;
    uint8_t noWait = 0;
    //uint8_t count_vgm[BUF_CMDS];
    uint8_t count_incoming[BUF_CMDS];
    ...
    USBUART_PutString("\0");
    // fill buffers
    for (k = 0; k < BUF_CMDS; k++)
    {
        bufPtr = buffer[k];
        while (!rx_flag);
        count_incoming[k] = USBUART_GetAll(bufPtr);
        //count_vgm = count_incoming;
        rx_flag = 0;
    }

    i = 0; // start playback from 1st buffer
    bufPtr = buffer[0];
    VGMPtr = buffer[0];
    ...
    { 
        if (isrFlag == 1)
        {
            ...
            case 0x7E:
            case 0x7F:
                // WARNING: this is different from VGM specs
            ...
            // buffer end
            if (j >= count_incoming[i] - 1)
            {   
                VGMPtr = buffer[(i + 1) % BUF_CMDS]; // change buffer and...
                // ... get incoming data from USB
                while (!rx_flag);
                //count_vgm = count_incoming;
                //count_incoming = USBUART_GetAll(bufPtr);
                count_incoming[i] = USBUART_ReadOutEP(3, bufPtr, 64);
                incomingFlag = 1;
                rx_flag = 0;
                bufPtr = VGMPtr;
                i = (i + 1) % BUF_CMDS;
